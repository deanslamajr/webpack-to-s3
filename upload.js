...
const workingDirectory = path.join(__dirname, 'public');
...
files.forEach(file => {
  ...
  // gzipped files
  if (/\.(gz)/.test(file)) {
    tasks.push(uploadFile(workingDirectory, file, { ...s3Config, ...gzipMetaTags }));
  }
  // css files
  else if (/\.(css)/.test(file)) {
    tasks.push(uploadFile(workingDirectory, file, { ...s3Config, ...cssMetaTags }));
  }
  // file types to skip uploading
    ...
    tasks.push(uploadFile(workingDirectory, file, s3Config));
  }
});
